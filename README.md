...
| レイヤー | 技術 |
|---------|------|
| **バックエンド** | Hono (TypeScript) on Cloudflare Pages Functions |
...
| **ビルド** | Vite + @hono/vite-build (Cloudflare Pages 向け) |
...
| title | TEXT | タイトル |
| content | TEXT | 原文テキスト |
| source_type | TEXT | ファイル種別（txt / pdf / docx / paste） |
...
reeda/
├── src/
│   ├── index.tsx              # Hono アプリ エントリ + HTML Shell
│   ├── types.ts               # 型定義（Env バインディング）
│   ├── routes/
│   │   ├── auth.ts            # 認証 API（register / login / logout / me）
│   │   └── documents.ts       # ドキュメント API（CRUD / 進捗 / ブックマーク）
│   └── lib/
│       ├── analyzer.ts        # 形態素解析（Vite ビルド用、未使用）
